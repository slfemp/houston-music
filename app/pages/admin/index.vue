...
useHead({ title: 'HMAB Admin — Sign In' })

const { data: me } = await useFetch('/api/auth/me')
watchEffect(() => {
  if (me.value?.admin) navigateTo('/admin/network')
})
  ...
  <div class="min-h-screen flex items-center justify-center px-6">
    <div class="w-full max-w-sm">
      <div class="text-center mb-8">
        <div class="text-2xl font-bold tracking-wide">HMAB <span class="text-electric-blue">ADMIN</span></div>
      ...
      <div v-else class="bg-space-gray rounded-2xl p-6 border border-white/10 text-center">
        <p class="text-sm">If that address is on the board allowlist, a sign-in link is on its way. It expires in 15 minutes.</p>
