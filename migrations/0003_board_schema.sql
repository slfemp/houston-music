-- Migration number: 0003    Board console schema (ported from nuxt4-upgrade drizzle migrations)
-- from 0000_boring_shen
...
CREATE INDEX `agenda_meeting_idx` ON `agenda_items` (`meeting_id`,`position`);
...
CREATE UNIQUE INDEX `attendance_unique` ON `attendance` (`meeting_id`,`member_id`);
...
CREATE UNIQUE INDEX `budget_unique` ON `budget_lines` (`fiscal_year`,`category`,`kind`);
...
CREATE UNIQUE INDEX `event_rsvp_unique` ON `event_rsvps` (`event_id`,`user_id`);
CREATE TABLE `events` (
	`id` integer PRIMARY KEY AUTOINCREMENT NOT NULL,
	`title` text NOT NULL,
...
CREATE INDEX `events_visibility_idx` ON `events` (`visibility`,`published`);
...
CREATE UNIQUE INDEX `issue_support_unique` ON `issue_support` (`issue_id`,`user_id`);
CREATE TABLE `issues` (
	`id` integer PRIMARY KEY AUTOINCREMENT NOT NULL,
	`title` text NOT NULL,
	`description` text NOT NULL,
...
CREATE INDEX `members_seated_idx` ON `members` (`seated`);
...
CREATE INDEX `motions_meeting_idx` ON `motions` (`meeting_id`);
CREATE INDEX `motions_agenda_idx` ON `motions` (`agenda_item_id`);
...
CREATE INDEX `tx_account_date_idx` ON `transactions` (`account_id`,`occurred_on`);
...
CREATE INDEX `treas_meeting_idx` ON `treasurer_reports` (`meeting_id`);
CREATE TABLE `users` (
	`id` integer PRIMARY KEY AUTOINCREMENT NOT NULL,
	`email` text NOT NULL,
	`name` text NOT NULL,
...
CREATE INDEX `vol_opps_published_idx` ON `volunteer_opportunities` (`published`,`starts_at`);
...
CREATE UNIQUE INDEX `signup_unique` ON `volunteer_signups` (`opportunity_id`,`email`);
CREATE INDEX `signup_opp_idx` ON `volunteer_signups` (`opportunity_id`);
CREATE TABLE `votes` (
	`id` integer PRIMARY KEY AUTOINCREMENT NOT NULL,
...
CREATE UNIQUE INDEX `votes_unique` ON `votes` (`motion_id`,`member_id`);-- from 0001_fast_talon
...
CREATE INDEX `merch_published_idx` ON `merch_items` (`published`,`sort_order`);-- from 0002_bright_rawhide_kid
...
CREATE INDEX `venues_published_idx` ON `booking_venues` (`published`,`name`);
CREATE INDEX `venues_type_idx` ON `booking_venues` (`venue_type`);
...
CREATE UNIQUE INDEX `musicians_email_idx` ON `musicians` (`email`);
CREATE INDEX `musicians_status_idx` ON `musicians` (`status`,`listed`);-- from 0003_furry_sunspot
ALTER TABLE `attendance` ADD `acting_for_member_id` integer;